...
class DefaultController extends Controller {
	...
	public function actionCreate() {
		if (Yii::app ()->authManager->checkAccess ( $this->formid . '-Append', Yii::app ()->user->id )) {
			...
			$model = new Acctperiods ();
			$this->afterInsert ( $model );
			
			// Uncomment the following line if AJAX validation is needed
			$this->performAjaxValidation ( $model );
			
			if (isset ( $_POST ['Acctperiods'] )) {
				$model->attributes = $_POST ['Acctperiods'];
	...
	public function actionUpdate($id) {
		if (Yii::app ()->authManager->checkAccess ( $this->formid . '-Update', Yii::app ()->user->id )) {
			...
			if (isset ( $_POST ['Acctperiods'] )) {
				...
				$this->beforePost ( $model );
				$this->tracker->modify ( 'acctperiods', $id );
				if ($model->save ()) {
					$this->afterPost ( $model );
					$this->redirect ( array (
							'view',
	...
	public function actionDelete($id) {
		if (Yii::app ()->authManager->checkAccess ( $this->formid . '-Delete', Yii::app ()->user->id )) {
			$this->trackActivity ( 'd' );
			$model = $this->loadModel ( $id );
			$this->beforeDelete ( $model );
			$this->tracker->delete ( 'acctperiods', $id );
			$model->delete ();
			$this->afterDelete ();
	...
	public function actionIndex() {
		if (Yii::app ()->authManager->checkAccess ( $this->formid . '-List', Yii::app ()->user->id )) {
			...
			$dataProvider = new CActiveDataProvider ( 'Acctperiods' );
	...
	public function actionAdmin() {
		if (Yii::app ()->authManager->checkAccess ( $this->formid . '-List', Yii::app ()->user->id )) {
			...
			$model = new Acctperiods ( 'search' );
			$model->unsetAttributes (); // clear any default values
			if (isset ( $_GET ['Acctperiods'] ))
				$model->attributes = $_GET ['Acctperiods'];
	...
	public function actionRestore($idtrack) {
		if (Yii::app ()->authManager->checkAccess ( $this->formid . '-Update', Yii::app ()->user->id )) {
			...
			$this->tracker->restore ( 'acctperiods', $idtrack );
			$dataProvider = new CActiveDataProvider ( 'Acctperiods' );
			$this->render ( 'index', array (
					'dataProvider' => $dataProvider 
			) );
	...
	public function actionRestoreDeleted($idtrack) {
		if (Yii::app ()->authManager->checkAccess ( $this->formid . '-Update', Yii::app ()->user->id )) {
			...
			$this->tracker->restoreDeleted ( 'acctperiods', $idtrack );
			$dataProvider = new CActiveDataProvider ( 'Acctperiods' );
			$this->render ( 'index', array (
					'dataProvider' => $dataProvider 
			) );
	 ...
	 * @return Acctperiods the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id) {
		$model = Acctperiods::model ()->findByPk ( $id );
		if ($model === null)
	 ...
	 * 
	 * @param Acctperiods $model
	 *        	the model to be validated
	 */
	protected function performAjaxValidation($model) {
		if (isset ( $_POST ['ajax'] ) && $_POST ['ajax'] === 'acctperiods-form') {
	...
	protected function afterInsert(& $model) {
		...
		$model->startdate = $idmaker->getDateTime();
		$model->enddate = $idmaker->getDateTime();
